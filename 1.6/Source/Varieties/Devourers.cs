    ...
    public class SitePartWorker_BetterDistressCall_Devourers : SitePartWorker_DistressCall
    {
        public override void PostMapGenerate(Map map)
        {
            ...
            List<Pawn> list = PawnGroupMakerUtility.GeneratePawns(new PawnGroupMakerParms
            {
                ...
                points = site.ActualThreatPoints * BetterDistressCall_Settings.DevourersSettlementPointModifier,
                tile = map.Tile,
                inhabitants = true,
            ...
            float num = Faction.OfEntities.def.MinPointsToGeneratePawnGroup(PawnGroupKindDefOf.Devourers) * 1.05f;
            List<Pawn> Devourers = PawnGroupMakerUtility.GeneratePawns(new PawnGroupMakerParms
            {
            ...
            {
                groupKind = PawnGroupKindDefOf.Devourers,
                points = site.ActualThreatPoints * BetterDistressCall_Settings.DevourersPointModifier,
                faction = Faction.OfEntities,
                raidStrategy = RaidStrategyDefOf.ImmediateAttack
            }).ToList();
            Lord lord = LordMaker.MakeNewLord(Faction.OfEntities, new LordJob_ChimeraAssault(), map, list2.Concat(Devourers));
            ...
            if (ticks < 30000)
            {
                ...
                DistressCallUtility.SpawnPawns(map, Devourers.Concat(list2), map.Center, 40);
                if (lord.LordJob is LordJob_ChimeraAssault lordJob_ChimeraAssault)
                {
                    lordJob_ChimeraAssault.SwitchMode();
                }
                Lord lord2 = LordMaker.MakeNewLord(faction, new LordJob_AssaultThings(Faction.OfEntities, new List<Thing>(list2.Concat(Devourers).ToList()), 0.4f, false), map, list);
                stage = 1;
            }
            else if (ticks < 60000)
            {
                ...
                BetterDistressCallHelper.WoundPawns(woundedPawns, Devourers.Concat(list2).ToList());
                ...
                DistressCallUtility.SpawnCorpses(map, Devourers, list, map.Center, 40);
                if (lord.LordJob is LordJob_ChimeraAssault lordJob_ChimeraAssault)
                {
                    lordJob_ChimeraAssault.SwitchMode();
                }
                Lord lord2 = LordMaker.MakeNewLord(faction, new LordJob_AssaultThings(Faction.OfEntities, new List<Thing>(list2.Concat(Devourers).ToList()), 1, false), map, list);
                stage = 2;
            ...
            else if (ticks < 120000)
            {
                ...
                DistressCallUtility.SpawnPawns(map, list, map.Center, 20);
                DistressCallUtility.SpawnPawns(map, woundedPawns, map.Center, 20);
                DistressCallUtility.SpawnCorpses(map, deadPawns, Devourers.Concat(list2), map.Center, 20);
                DistressCallUtility.SpawnPawns(map, list2, map.Center, 40);
                DistressCallUtility.SpawnCorpses(map, Devourers, list, map.Center, 20);
                if (lord.LordJob is LordJob_ChimeraAssault lordJob_ChimeraAssault)
                {
                    lordJob_ChimeraAssault.SwitchMode();
                }
            ...
            else if (ticks < 180000)
            {
                ...
                BetterDistressCallHelper.   WoundPawns(woundedPawns, Devourers.Concat(list2).ToList());
                DistressCallUtility.SpawnPawns(map, woundedPawns, map.Center, 20);
                DistressCallUtility.SpawnCorpses(map, list, Devourers.Concat(list2), map.Center, 20);
                DistressCallUtility.SpawnPawns(map, list2, map.Center, 40);
                DistressCallUtility.SpawnCorpses(map, Devourers, list, map.Center, 20);
                stage = 4;
            }
            ...
            EnterSendLetter.SendLetter(stage.ToString(), "Devourers", faction, Obelisk);
