...
namespace BetterDistressCall.Varieties
{
    public class SitePartWorker_BetterDistressCall_Shamblers : SitePartWorker_DistressCall
    {
        public override void PostMapGenerate(Map map)
        {
            ...
            List<Pawn> shamblers = PawnGroupMakerUtility.GeneratePawns(new PawnGroupMakerParms
            {
            ...
            if (ticks < 30000)
            {
                ...
                DistressCallUtility.SpawnPawns(map, shamblers.Concat(list2), map.Center, 20);
                Lord lord2 = LordMaker.MakeNewLord(faction, new LordJob_AssaultThings(Faction.OfEntities, new List<Thing>(list2.Concat(shamblers).ToList()), 0.4f, false), map, list);
                stage = 1;
            }
            else if (ticks < 60000)
            {
                ...
                DistressCallUtility.SpawnCorpses(map, shamblers, list, map.Center, 20);
                Lord lord2 = LordMaker.MakeNewLord(faction, new LordJob_AssaultThings(Faction.OfEntities, new List<Thing>(list2.Concat(shamblers).ToList()), 0.4f, false), map, list);
